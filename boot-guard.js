...
function installStartupFetchGuard(){
  if(window.__nvStartupFetchGuardInstalled) return;
  window.__nvStartupFetchGuardInstalled=true;
  const originalFetch=window.fetch.bind(window);
  window.fetch=(input,init={})=>{
    const isBoot=document.querySelector('#app .boot');
    if(!isBoot) return originalFetch(input,init);
    ...
    timer=setTimeout(()=>controller.abort(new DOMException('Startup request timeout','TimeoutError')),STARTUP_TIMEOUT_MS);
...
repairLocalStorage();
installStartupFetchGuard();
window.addEventListener('error',e=>{if(bootEl())showError(e.error?.message||e.message||'JavaScript error')});
window.addEventListener('unhandledrejection',e=>{if(bootEl())showError(e.reason?.message||String(e.reason||'Promise rejection'))});
setTimeout(()=>{
  if(bootEl()) showError('Ứng dụng không hoàn tất khởi động trong thời gian cho phép.');
},8000);
})();