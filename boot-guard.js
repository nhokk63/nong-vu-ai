...
function repairLocalStorage(){
  for(const key of JSON_KEYS){
    try{const raw=localStorage.getItem(key);if(raw==null||raw==='')continue;JSON.parse(raw)}catch{try{localStorage.removeItem(key)}catch{}}
  }
}
...
setTimeout(()=>{if(bootEl())showError('Ứng dụng không hoàn tất khởi động trong thời gian cho phép.')},8000);
})();
