const VERSION = 'nongvu-ai-v26.0';
const CACHE = `${VERSION}-static`;
const STATIC = [
  './styles.css?v=44',
  './manifest.json',
  './knowledge.json',
  './chemical-catalog.json?v=20260820',
  './icon-180.png',
  './icon-192.png',
  './icon-512.png'
];
...
self.addEventListener('install', event => {
  event.waitUntil(
    caches.open(CACHE).then(c => c.addAll(STATIC)).catch(() => {}).finally(() => self.skipWaiting())
  );
});
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith(
    caches.match(req).then(cached => cached || fetch(req).then(res => {
      if (res.ok && url.origin === location.origin) caches.open(CACHE).then(c => c.put(req, res.clone())).catch(() => {});
      return res;
    }))
  );
});
