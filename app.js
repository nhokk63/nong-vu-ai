...
function calendarView(){
 const a=[...state.tasks].sort((x,y)=>(x.scheduled_at||'').localeCompare(y.scheduled_at||''));
 if(!a.length)return `<div class="card empty">Chưa có việc. Duyệt một khuyến cáo để tạo lịch.</div>`;
 const fmt=t=>t.scheduled_at?new Date(t.scheduled_at).toLocaleString('vi-VN',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}):'Chưa đặt';
 const countdown=t=>{if(t.status==='DONE')return '✓ Đã hoàn thành'; if(!t.scheduled_at)return 'Chưa có thời gian'; const d=new Date(t.scheduled_at)-Date.now(),m=Math.round(Math.abs(d)/60000),days=Math.floor(m/1440),h=Math.floor((m%1440)/60),mm=m%60; const s=days?`${days} ngày ${h} giờ`:h?`${h} giờ ${mm} phút`:`${mm} phút`; return d>=0?`Còn ${s}`:`Quá ${s}`};
 return `<div class="section"><div class="card"><div class="section-title">Lịch chăm sóc</div><div class="note" style="margin-top:4px">Bấm vào từng việc để xem đầy đủ yêu cầu, mốc thời gian và cập nhật tình trạng cây.</div></div></div><div class="list">${a.map(t=>{const p=state.plants.find(x=>x.id===t.plant_id);const over=t.status!=='DONE'&&t.scheduled_at&&new Date(t.scheduled_at)<new Date();const tone=t.status==='DONE'?'green':over?'red':t.status==='POSTPONED'?'orange':'blue';return `<div class="item task-card" data-open-task="${esc(t.id)}"><div class="row"><div><div class="item-title">${esc(t.title||t.kind||'Công việc')}</div><div class="item-meta">${esc(p?.name||'Cây trồng')} · ${esc(fmt(t))}</div></div><span class="tag ${tone}">${esc(t.status)}</span></div><div class="task-countdown ${t.status==='DONE'?'done':over?'overdue':''}" data-countdown-id="${esc(t.id)}" data-countdown-time="${esc(t.scheduled_at||'')}">${esc(countdown(t))}</div><div class="task-hint">Chạm để xem yêu cầu · sau khi làm nhớ cập nhật cây</div><div class="actions">${t.status!=='DONE'?`<button class="btn success" data-action="done-task" data-id="${esc(t.id)}">✓ Đã làm</button><button class="btn secondary" data-action="postpone-task" data-id="${esc(t.id)}">Hoãn</button>`:''}</div></div>`}).join('')}</div>`;
}
...
async function consult(idPlant){
 ...
 e.addEventListener('click',async x=>{
  ...
  try{
    ...
    const body={plant:p,observation:$('#obs',e).value.trim(),inventory:state.inventory,weather:w,history:{recommendations:state.recs.filter(r=>r.plant_id===p.id && r.status!=='REJECTED').slice(0,5),tasks:state.tasks.filter(t=>t.plant_id===p.id && t.status!=='DONE').slice(0,5)},knowledge:state.knowledge,image};
    let r;
    try{
...
document.addEventListener('click',e=>{const item=e.target.closest('[data-open-task]');if(item && !e.target.closest('[data-action]')){openTaskDetail(item.dataset.openTask)}},{capture:true});
setInterval(()=>{if(state.tab==='calendar')document.querySelectorAll('[data-countdown-time]').forEach(el=>{const d=new Date(el.dataset.countdownTime)-Date.now();if(!Number.isFinite(d))return;const m=Math.round(Math.abs(d)/60000),days=Math.floor(m/1440),h=Math.floor((m%1440)/60),mm=m%60,s=days?`${days} ngày ${h} giờ`:h?`${h} giờ ${mm} phút`:`${mm} phút`;el.textContent=d>=0?`Còn ${s}`:`Quá ${s}`;el.classList.toggle('overdue',d<0)})},30000);
...
function openTaskDetail(taskId){
 const t=state.tasks.find(x=>x.id===taskId); if(!t)return; const p=state.plants.find(x=>x.id===t.plant_id); let meta={}; try{meta=t.meta?JSON.parse(t.meta):{}}catch{}; const notes=(t.notes||meta.notes||'').split(/\n+/).map(x=>x.trim()).filter(Boolean);
 const fmt=iso=>iso?new Date(iso).toLocaleString('vi-VN',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}):'Chưa đặt';
 const cd=()=>{if(t.status==='DONE')return '✓ Đã hoàn thành';if(!t.scheduled_at)return 'Chưa đặt thời gian';const d=new Date(t.scheduled_at)-Date.now(),m=Math.round(Math.abs(d)/60000),days=Math.floor(m/1440),h=Math.floor((m%1440)/60),mm=m%60,s=days?`${days} ngày ${h} giờ`:h?`${h} giờ ${mm} phút`:`${mm} phút`;return d>=0?`Còn ${s}`:`Quá ${s}`};
 const e=document.createElement('div');e.className='nv-detail';e.innerHTML=`<div style="width:100%"><div class="sheet" style="position:relative"><button class="nv-close" data-close-task>×</button><h2 style="padding-right:50px">${esc(t.title||t.kind||'Công việc')}</h2><div class="item-meta" style="margin-top:4px">${esc(p?.name||'Cây trồng')} · ${esc(stageName(p?.crop,p?.stage))}</div><div class="nv-live" data-live-task="1">${cd()}</div><div class="nv-req"><b>Thời gian</b><div class="note" style="margin-top:5px">${esc(fmt(t.scheduled_at))}</div></div><div class="nv-req"><b>Yêu cầu thực hiện</b>${notes.length?`<ul>${notes.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:`<div class="note" style="margin-top:6px">Chưa có ghi chú chi tiết.</div>`}</div><div class="nv-req"><b>Sau khi làm</b><div class="card" style="margin-top:8px;padding:12px;background:#f7f7f9">Ghi nhận kết quả, tình trạng cây và ảnh nếu có. AI sẽ dùng cập nhật này cho lần đánh giá sau.</div></div><div class="actions" style="margin-top:16px">${t.status!=='DONE'?`<button class="btn success" data-task-done="1">✓ Đã thực hiện</button><button class="btn secondary" data-task-postpone="1">Hoãn 1 ngày</button>`:''}<button class="btn primary" data-task-plant="1">Cập nhật cây</button></div></div></div>`;
 document.body.appendChild(e);
 const timer=setInterval(()=>{const q=e.querySelector('[data-live-task]');if(q)q.textContent=cd()},30000);
 e.addEventListener('click',async ev=>{if(ev.target.closest('[data-close-task]')||ev.target===e){clearInterval(timer);e.remove();return}if(ev.target.closest('[data-task-done]')){t.status='DONE';t.completed_at=now();await saveChanges();clearInterval(timer);e.remove();render();toast('Đã hoàn thành. Nhớ cập nhật tình trạng cây.','success');return}if(ev.target.closest('[data-task-postpone]')){t.scheduled_at=new Date(new Date(t.scheduled_at).getTime()+86400000).toISOString();t.status='PLANNED';await saveChanges();clearInterval(timer);e.remove();render();toast('Đã hoãn 1 ngày','success');return}if(ev.target.closest('[data-task-plant]')){clearInterval(timer);e.remove();location.hash='plant';render();}});
}
