...
function weatherCard(){
  const w=state.weather?.current; if(!w) return `<div class="section"><div class="weather-empty card"><div class="weather-empty-icon">${svg('pin')}</div><div><div class="section-title">Thời tiết & vị trí</div><div class="note">Chưa có dữ liệu. Bấm định vị để lấy thời tiết hiện tại.</div></div><button class="btn primary" data-action="refresh-weather">Định vị</button></div></div>`;
  ...
  const forecasts=(daily.time||[]).slice(0,5).map((date,i)=>{const q=weatherInfo(daily.weather_code?.[i] ?? 0,1); return `<div class="forecast-cell"><div class="forecast-day">${esc(dayLabel(date))}</div><div class="forecast-icon ${q.accent}">${weatherIcon(q.icon)}</div><div class="forecast-temp">${Math.round(daily.temperature_2m_max?.[i]??0)}°</div><div class="forecast-low">${Math.round(daily.temperature_2m_min?.[i]??0)}°</div><div class="forecast-rain">${Math.round(daily.precipitation_probability_max?.[i]??0)}%</div></div>`}).join('');
  return `<section class="section weather-section"><div class="weather-card ${wi.key}">
    <div class="weather-top"><div><div class="weather-kicker">THỜI TIẾT HIỆN TẠI</div><div class="weather-place">${loc.name?esc(loc.name):'Vị trí hiện tại'}</div><div class="weather-coord">${formatCoord(loc.lat,true)} · ${formatCoord(loc.lon,false)}</div></div><button class="weather-locate" data-action="refresh-weather" aria-label="Cập nhật vị trí">${svg('pin')}</button></div>
    <div class="weather-mainline"><div class="weather-art">${weatherIcon(wi.icon)}</div><div><div class="weather-temp-xl">${Math.round(Number(w.temperature_2m||0))}°</div><div class="weather-condition">${wi.label}</div></div></div>
    <div class="weather-metrics"><div><span>Độ ẩm</span><strong>${hum}%</strong></div><div><span>Mưa</span><strong>${rain.toFixed(1)} mm</strong></div><div><span>Gió</span><strong>${Math.round(wind)} km/h</strong></div><div><span>Gió giật</span><strong>${Math.round(Number(w.wind_gusts_10m||0))} km/h</strong></div></div>
    <div class="weather-note">Dữ liệu theo vị trí GPS đã lưu. AI dùng thời tiết này khi tư vấn và đánh giá cửa sổ xử lý.</div>
    <div class="forecast-row">${forecasts || '<div class="note">Chưa có dự báo 5 ngày.</div>'}</div>
  </div></section>`;
}
...
async function refreshWeather(){
  try{
    ...
    const pos=await new Promise((res,rej)=>navigator.geolocation?.getCurrentPosition(res,rej,{enableHighAccuracy:true,timeout:12000,maximumAge:30000}) || rej(new Error('Trình duyệt không hỗ trợ GPS')));
...
function addPlantModal(){
 ...
 e.addEventListener('click',async x=>{ if(x.target.closest('[data-close]'))return e.remove(); if(!x.target.closest('[data-save-plant]'))return; const btn=x.target.closest('[data-save-plant]'); btn.disabled=true; try{
  const pos=await new Promise((res,rej)=>navigator.geolocation?.getCurrentPosition(res,rej,{enableHighAccuracy:true,timeout:10000,maximumAge:30000}) || rej(new Error('Trình duyệt không hỗ trợ GPS')));
